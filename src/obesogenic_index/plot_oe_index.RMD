...
final.df <- obesity.std %>%
  dplyr::select(id_type, no_insurance, hispanic, no_highschool,
...
factor1 <- final.df %>% dplyr::select(no_insurance, no_highschool, hispanic, black, poverty)
...
psych::alpha(factor1) # alpha = 0.89 
psych::alpha(factor2) # alpha = 0.92 
psych::alpha(factor3) # alpha = 0.82
...
plot_index(highschool.shp, "High School Attendance Area", "Obesogenic Environment")
plot_index(supervisor.shp, "Supervisor District", "Obesogenic Environment")
```
...
for(i in 1:nrow(obesogenic.result.df)) {
  ggsave(sprintf("./src/obesogenic_index/figures/19_8_4_%s.jpg",
